...
from base_iris_lab2 import train, new_model, score, add_dataset, test
...
@app.route(base_url + "model/<int:model_id>/test", methods=['GET'])
def test_model(model_id: int):
    datasetid = int(request.args.get("dataset", type = int))

    if datasetid is None:
        return Response("fields are required", status=400)

    try:
        the_test = test(model_id, datasetid)
        return jsonify(the_test)
